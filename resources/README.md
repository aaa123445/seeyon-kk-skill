...
## Image assets and OCR

Some online documentation content is embedded in screenshots/diagrams. The reusable image resources are bundled here:

```text
resources/images/          # downloaded images from online docs
resources/image-index.json # source URL, section, local file, OCR metadata
resources/image-ocr.md     # raw OCR text from Tesseract
knowledge/image-derived-insights.md # curated visual interpretation
```

Use `knowledge/image-derived-insights.md` for implementation guidance. Treat `resources/image-ocr.md` as raw evidence only; OCR may misread low-resolution screenshots.
